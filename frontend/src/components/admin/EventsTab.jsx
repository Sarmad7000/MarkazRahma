          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-col gap-2 mt-1">
                ...
                />
                {uploadingNew && <p className="text-sm text-cyan-600">Uploading...</p>}
                {newEvent.image_path && (
                  <img src={newEvent.image_path} alt="Preview" className="max-h-40 object-contain rounded border" />
                )}
              </div>
            </div>
                ...
                >
                  {editingId === event.id ? (
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          className="mt-1"
                        />
                        {uploadingEdit && <p className="text-sm text-cyan-600">Uploading...</p>}
                      </div>
                      {editingEvent.image_path && (
                        <img src={editingEvent.image_path} alt="Preview" className="max-h-40 object-contain rounded border" />
                      )}
                      <div className="flex gap-2">
                        <Button size="sm" onClick={() => saveEdit(event.id)} className="flex-1">
                          <Save className="h-4 w-4 mr-1" /> Save
                        </Button>
                        <Button size="sm" onClick={cancelEdit} variant="outline" className="flex-1">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      {event.image_path && (
                        <img src={event.image_path} alt={event.title} className="w-full sm:w-32 h-24 object-cover rounded" />
                      )}
                      <div className="flex-1 min-w-0">
                        <h4 className={`font-semibold mb-1 ${event.enabled ? '' : 'line-through'}`}>{event.title}</h4>
